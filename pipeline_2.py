...
def pairwise():
  ...
  job_list = []
  #submit_jobs('check_lsq6', 's1*', './process.py checking', job_list, 4, 0, 0,0)
  num = 0
  for subject in glob.glob('inputs/*'):
    sourcename = subject[7:11]
...
def nonlinregs():
  ...
  #submit_jobs('check_reg2', 'nonlin2avg', './process.py check_reg 2', job_list)
  
  nonlinreg_and_avg('3', 'timages_nonlin', 'nonlin2', 'nonlin2avg.mnc', '100x5')
  #submit_jobs('check_reg3', 'nonlin3avg', './process.py check_reg 3', job_list)
  
  nonlinreg_and_avg('4', 'timages_nonlin', 'nonlin3', 'nonlin3avg.mnc', '5x20')
  #submit_jobs('check_reg4', 'nonlin4avg', './process.py check_reg 4', job_list)
  return
