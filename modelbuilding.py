...
""" 
Determines which stages of the Model-Building Pipeline need to be executed
...
def call_preprocess():
  ...
  for inputname in listofinputs:
    if not os.path.exists('%s/output_lsq6/%s_lsq6.mnc' %(inputname, inputname)):
      job_list.append('./modelbuilding_process.py preprocess %s %s %s' %(inputname, image_type, target_type))
...
def call_preprocess2():
  ...
  if len(glob.glob('*/NORM/*_crop.mnc')) == 0:
    job_list = ['./modelbuilding_process.py autocrop %s %s' %(image_type,targetname)]
  ...
  for sourcename in listofinputs:
    if not os.path.exists('%s/output_lsq6/%s_lsq6.mnc' %(sourcename, sourcename)):
      job_list.append('./modelbuilding_process.py preprocess2 %s %s %s' %(sourcename, targetname, image_type))
...
def nonpairwise():
    ...
    for sourcename in listofinputs:     
      if sourcename != targetname: 
        if not os.path.exists('%s/lin_tfiles/%s_%s_lsq12.xfm' %(sourcename, sourcename, targetname)):
          job_list.append('./modelbuilding_process.py lsq12_reg %s %s lin_tfiles' %(sourcename, targetname))
    submit_jobs('s2', 's1*', job_list)
        
  # PART 2: calls xfmavg_inv_resample
  job_list = ['./modelbuilding_process.py xfmavg_inv_resample %s' %targetname]
  if not os.path.exists('avgsize.mnc'):
  ...
  for sourcename in listofinputs:    
    if not os.path.exists('%s/output_lsq12/%s_lsq12.mnc' %(sourcename, sourcename)):
      job_list.append('./modelbuilding_process.py lsq12reg_and_resample %s' %sourcename)
...
def pairwise():
  ...
  for sourcename in listofinputs:
    for targetname in listofinputs:
      if sourcename != targetname and not os.path.exists('%s/pairwise_tfiles/%s_%s_lsq12.xfm' %(sourcename, sourcename, targetname)):
        job_list.append('./modelbuilding_process.py lsq12_reg %s %s pairwise_tfiles' %(sourcename, targetname))      
  submit_jobs('s2', 's1*', job_list)
  ...
  for inputname in listofinputs:
    if not os.path.exists('%s/output_lsq12/%s_lsq12.mnc' %(inputname,inputname)):
      job_list.append('./modelbuilding_process.py xfmavg_and_resample %s' %inputname)
...
def call_linavg():
  ...
  job_list = ['./modelbuilding_process.py linavg_and_check output_lsq12 lsq12 linavg.mnc']
...
def ANTS_and_avg(number, sourcefolder,inputregname, targetimage, iterations):
  ...
  for inputname in listofinputs:
    if not os.path.exists('%s/nlin_timages/%s_nlin%s.mnc' %(inputname,inputname, number)):
      job_list.append('./modelbuilding_process.py ants_and_resample %s %s/%s/%s_%s.mnc %s %s %s' 
 ...
 # PART 2: calls mnc_avg
  job_list = ['./modelbuilding_process.py mnc_avg nlin_timages nlin%s nlin%savg.mnc' %(number, number)]
...
def tracc_resmp(stage, fwhm, iterations, step, model):
  ...
  job_list =['./modelbuilding_process.py model_blur %s %s' %(fwhm, model)]
  if not os.path.exists('avgimages/nlin%savg_tracc.mnc' %stage):
    ...
    job_list = []
    for inputname in listofinputs:
      job_list.append('./modelbuilding_process.py tracc %s %s %s %s %s %s' %(inputname, stage, fwhm, iterations, step, model))
    submit_jobs('tr%s' %stage, 'blurmod%s*' %stage, job_list)
    
    # PART 3: calls mnc_avg
    job_list = ['./modelbuilding_process.py mnc_avg minctracc_out nlin%s nlin%savg_tracc.mnc' %(stage, stage)]
...
def call_final_stats():
  ...
  for inputname in listofinputs:
    if not os.path.exists('%s/final_stats/%s_det_8_blur.mnc' %(inputname, inputname)):
      job_list.append( './modelbuilding_process.py deformation %s' %inputname)
...
def call_longitudinal():
  ...
  for inputname_time2 in listofinputs_time2:
    ...
    if not os.path.exists('%s/longitudinal/det_fwhm8_blur.mnc' %inputname_time2[0:-2]):
      job_list.append('./modelbuilding_process.py longitudinal %s' %inputname_time2)
...
def landmark():
  ...
  job_list = ['./modelbuilding_process.py tag_nlinavg']
  ...
  for inputname in listofinputs:
    if not os.path.exists('%s/%s_size_shape_diff_landmarks.csv' %(inputname, inputname)):
      job_list.append('./modelbuilding_process.py tag_subject %s' %inputname)
...
def call_asymm():
  ...
  for inputname in listofinputs:
    if not os.path.exists('%s/asymmetrical/%s_det_blur.mnc' %(inputname, inputname)):
      job_list.append('./modelbuilding_process.py asymmetric_analysis %s' %inputname)
...
if __name__ == '__main__':
  ...
  group.add_argument("-asymm", action="store_true", 
                      help="asymmetric analysis (lsq12 stage must be on the queue or complete)")
