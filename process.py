...
def preprocess(subject, image_type, target_type):
  ...
  if image_type == 'brain':
    ...
    execute('minccalc -clob %s/NUC/%s.mnc -expression "10000*(A[0]-0)/(%s-%s)" %s/NORM/%s.mnc' %(subject, subject, themax[0:-1], themin[0:-1], subject, subject)) 
    mask(subject, 'NORM') # use normalized image to generate the mask
    if target_type == 'given': # if target image is provided by the user 
      execute('bestlinreg -clob -lsq6 -source_mask %s/masks/mask.mnc -target_mask targetmask.mnc %s/NORM/%s.mnc targetimage.mnc %s/lin_tfiles/%s_lsq6.xfm' %(subject, subject, subject, subject, subject))  
      resample('%s/lin_tfiles/%s_lsq6.xfm' %(subject, subject), '%s/NORM/%s.mnc' %(subject, subject), '%s/output_lsq6/%s_lsq6.mnc' %(subject, subject))
  
  elif image_type == 'face':  
    mask(subject,'NUC') # use the nu_corrected image to generate the mask
    vartype = execute('mincinfo -vartype image %s/NUC/%s.mnc' %(subject,subject))
    ...
    if vartype == 'short\n':
      execute('minccalc -clob -expression "(1-A[1])*A[0]" %s/NUC/%s.mnc %s/masks/mask.mnc %s/NUC/%s_face.mnc' %(subject, subject, subject, subject, subject))
    elif vartype == 'long\n':
      execute('minccalc -clob -expression "(1-A[0])*A[1]" %s/masks/mask.mnc %s/NUC/%s.mnc %s/NUC/%s_face.mnc' %(subject, subject, subject, subject, subject))
    ...
    execute('minccalc -clob %s/NUC/%s_face.mnc -expression "10000*(A[0]-0)/(%s-%s)" %s/NORM/%s_face.mnc' %(subject, subject, themax[0:-1], themin[0:-1], subject, subject))
  return
...
def lsq12_reg(sourcename, targetname, outputfolder):
  """Pairwise and non-pairwise 12-parameter registrations: PART 1
  1) Estimate the linear 12-parameter transformation of each subject to all the other subjects (pairwise) or to the randomly selected target subject (non-pairwise)"""
  execute('bestlinreg -lsq12 %s/output_lsq6/%s_lsq6.mnc %s/output_lsq6/%s_lsq6.mnc %s/%s/%s_%s_lsq12.xfm' %(sourcename, sourcename, targetname, targetname, sourcename,outputfolder, sourcename, targetname))
  return
...
def xfmavg_and_resample(inputname):
  ...
  resample('%s/pairwise_tfiles/%s.xfm' %(inputname, inputname), '%s/output_lsq6/%s_lsq6.mnc' %(inputname, inputname), '%s/output_lsq12/%s_lsq12.mnc' %(inputname, inputname))
  return
...
def lsq12reg_and_resample(sourcename):
  ...
  resample('%s/lin_tfiles/%s_lsq12.xfm' %(sourcename, sourcename), '%s/output_lsq6/%s_lsq6.mnc' %(sourcename, sourcename), '%s/output_lsq12/%s_lsq12.mnc' %(sourcename, sourcename))
  return
...
def ants(inputname, sourcepath, targetimage, number, iterations):
  ...
  resample('%s/nonlin_tfiles/%s_nonlin%s.xfm' %(inputname, inputname, number), sourcepath, '%s/nonlin_timages/%s_nonlin%s.mnc' %(inputname,inputname,number))   
  return
...
def tracc(inputname, num, fwhm, iterations, step, model):
  ...
  if not os.path.exists('%s/minctracc_out/%s_lsq12_%s_blur.mnc' %(inputname, inputname,fwhm)):
    execute('mincblur -clob -fwhm %s %s/output_lsq12/%s_lsq12.mnc %s/minctracc_out/%s_lsq12_%s' %(fwhm, inputname, inputname, inputname, inputname,fwhm))
  ...
  resample('%s/minctracc_out/%s_out%s.xfm' %(inputname,inputname,num), '%s/output_lsq12/%s_lsq12.mnc' %(inputname, inputname), '%s/minctracc_out/%s_nlin%s.mnc' %(inputname,inputname,num))
  return
...
def deformation(inputname):
  ...
  try:
    ...
    execute('xfmjoin %s/nonlin_tfiles/%s_nonlin1.xfm %s/nonlin_tfiles/%s_nonlin2.xfm %s/nonlin_tfiles/%s_nonlin3.xfm %s/nonlin_tfiles/%s_nonlin4.xfm %s/%s_merged2.xfm' %(inputname, inputname, inputname, inputname, inputname, inputname, inputname, inputname, inputname, inputname))
    ...
    execute('minc_displacement %s/output_lsq12/%s_lsq12.mnc %s/%s_merged.xfm %s/final_stats/%s_grid.mnc' %(inputname, inputname, inputname, inputname, inputname, inputname))
