...
def preprocess(name, image_type, target_type):
  ...
  if image_type == 'brain':
    ...
    mask(name, 'NORM') # use normalized image to generate mask
    ...
    # by default minccalc takes the vartype of the first file on the command & the mask generated by sienax is 'long' by default
    # don't want to long before short?? 
    # TODO: check this! 
    if vartype == 'short\n':
...
def lsq6reg_and_resample(sourcename, targetname, image_type):   
  """Continuation of the preprocessing stage (executed when the target image is a randomly selected normalized input)
  1) Linear 6-parameter transformation (using the autocropped version of the randomly selected image 
  2) Resample
  """
  if image_type == 'brain':
    targetmask = '%s/masks/mask_crop.mnc' %targetname             # use the autocropped version of the randomly selected image's mask as the target mask  
