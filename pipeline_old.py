...
def submit_jobs_test(jobname, depends, job_list, batchsize, time, name_file): 
  if batch_system == 'sge':
...
def preprocessing():
  ...
  if target_type == 'given': 
    ...
    submit_jobs_test('s1_%s' %inputname, "something", job_list, 8, "2:00:00", 'preprocess')             # fix dependency
  elif target_type == 'random' or image_type == 'face':
    for subject in listofinputs:
      inputname = subject[0:-4]    
      if not os.path.exists('%s/masks/mask.mnc' %inputname):
        job_list.append('./process.py preprocess %s %s %s' %(inputname, image_type, target_type))
    submit_jobs_test('s1a_%s' %inputname, "something", job_list, 8, "2:00:00", 'preprocess_a') # fix dependency?
    #preprocessing_2()
