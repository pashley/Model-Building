...
def preprocessing_2():
  job_list =[]
  submit_jobs('s1b_%s' %targetname, 's1a_*', './process_face.py autocrop %s %s' %(image_type, targetname), job_list, 8, "1:00:00", 1,1, 'autocrop')
  ...
  for subject in listofinputs:
    ...
    if not os.path.exists('%s/output_lsq6/%s_lsq6.mnc' %(sourcename, sourcename)):
      num +=1
      submit_jobs('s1c_%s_%s' %(sourcename,targetname), 's1b*', './process_face.py lsq6reg_and_resample %s %s %s' %(sourcename, targetname, image_type),job_list, 8, "2:00:00", num, count-complete, 'preprocess_c')
  return
...
def nonpairwise():         # alternative to pairwise registrations (i.e. too many inputs)          
  ...
  for subject in listofinputs:
    ...
    if sourcename != targetname:
      if not os.path.exists('%s/lin_tfiles/%s_%s_lsq12.xfm' %(sourcename, sourcename, targetname)):
        num += 1
        submit_jobs('s2_%s' %sourcename, 's1*', './process_face.py lsq12reg %s %s' %(sourcename, targetname),job_list, 8, "2:00:00", num, count-complete-1, 'stage2a')
        #submit_jobs('sa_%s_%s' %(sourcename[2:4], targetname[2:4]), 's1_*', './process_face.py lsq12reg %s %s' %(sourcename, targetname),job_list, 8, "2:00:00", num, count-complete-1, 'stage2a')
  ...
  if not os.path.exists('avgsize.mnc'):
    submit_jobs('avgsize','s2_*', './process_face.py xfmavg_inv_resample %s' %targetname, job_list, 8, "2:00:00", 1, 1,'avgsize') 
  ...
  for subject in listofinputs: 
    ...
    if not os.path.exists('%s/timage_lsq12/%s_lsq12.mnc' %(sourcename, sourcename)):         
      num += 1
      submit_jobs('s3_%s' %sourcename, 'avgsize*', './process_face.py lsq12reg_and_resample %s' %sourcename, job_list, 8, "2:00:00", num, count-complete, 'stage3')
  return  
...
def pairwise():
  ...
  for subject in listofinputs:
    ...
    for subject2 in listofinputs:
      ...
      if sourcename != targetname and not os.path.exists('%s/pairwise_tfiles/%s_%s_lsq12.xfm' %(sourcename, sourcename, targetname)):
        num += 1
        submit_jobs('s2_%s_%s' %(sourcename[2:4], targetname[2:4]), 's1_*', './process_face.py pairwise_reg %s %s' %(sourcename, targetname), job_list, 8, "2:00:00", num, ((count-1)*count)-complete, 'plsq12')
  ...
  for subject in listofinputs:
    ...
    if not os.path.exists('%s/timage_lsq12/%s_lsq12.mnc' %(inputname,inputname)):
      num += 1
      submit_jobs('s3_%s' %inputname, 's2_*', './process_face.py xfmavg_and_resample %s' %inputname, job_list, 8, "2:00:00", num, count-complete, 'p_res')
  return
...
def linavg():
  ...
  if not os.path.exists('avgimages/linavg.mnc'):
    #submit_jobs('linavg', 's3_*', './process_face.py mnc_avg timage_lsq12 lsq12 linavg.mnc', job_list, 8, "1:00:00", 1, 1, 'linavg')
    submit_jobs('linavg', 's3_*', './process_face.py linavg_and_check timage_lsq12 lsq12 linavg.mnc %s' %string, job_list, 8, "1:00:00", 1, 1, 'linavg')

  #submit_jobs('check_lsq12', 'linavg', './process_face.py check_lsq12 %s' %string, job_list, 8, '1:00:00', 1,1)
  return
...
def nonlinreg_and_avg(number, sourcefolder,inputregname, targetimage, iterations):
  ...
  for subject in listofinputs:
    ...
    if not os.path.exists('%s/timages_nonlin/%s_nonlin%s.mnc' %(inputname,inputname, number)):
      num += 1
      submit_jobs('reg%s_%s' %(number, inputname), '%s_*' %targetimage[0:-4], './process_face.py nonlin_reg %s %s/%s/%s_%s.mnc %s %s %s' %(inputname, inputname, sourcefolder, inputname, inputregname, targetimage, number, iterations), job_list, 8, "2:00:00", num, count-complete, 'nlreg%s'%number)
  
  job_list = []
  # wait for all H*_nonlin#.mnc files    
  if not os.path.exists('avgimages/nonlin%savg.mnc' % number):
    submit_jobs('nonlin%savg' %number, 'reg%s_*' %number, './process_face.py mnc_avg timages_nonlin nonlin%s nonlin%savg.mnc' %(number, number), job_list, 8, "2:00:00",1,1, 'nlavg%s'%number)
  return
